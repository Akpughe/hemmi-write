...
interface StepStructureReviewProps {
  ...
  aiProvider: string;
  onConfirm: (structure: DocumentStructure, updatedSources?: ResearchSource[]) => void;
  onBack: () => void;
}

type RegenerationPhase = 'analyzing' | 'researching' | 'regenerating' | 'complete';
...
export default function StepStructureReview({
  ...
  const [regenerationPhase, setRegenerationPhase] = useState<RegenerationPhase>('analyzing');
  const [regenerationReport, setRegenerationReport] = useState<RegenerationReportType | null>(null);
  const [currentSources, setCurrentSources] = useState<ResearchSource[]>(sources);
  ...
  const handleRegenerate = async () => {
    if (!feedback.trim()) {
      alert('Please provide feedback on what you want to change in the structure.');
      return;
    }
  ...
  const deepRegenerateStructure = async (userFeedback: string) => {
    try {
      ...
      if (data.regenerationReport.newSourcesAdded.length > 0) {
        const allSources = [...currentSources, ...data.regenerationReport.newSourcesAdded];
  ...
  if (error && !structure) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <h3 className="text-xl font-semibold text-gray-900 mb-2">Structure Generation Failed</h3>
  ...
  if (isLoading || !structure) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-gray-600 text-lg">Planning your document structure...</p>
        <p className="text-gray-500 text-sm mt-2">Analyzing sources and creating outline</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="font-semibold text-blue-900 mb-1">Review the Writing Plan</h4>
            <p className="text-sm text-blue-800">
              This is our approach for writing your {config.label.toLowerCase()}. You can provide feedback to refine the structure before we start writing.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-6">
        {/* Title */}
        <div>
          <h3 className="text-xl font-bold text-gray-900 mb-2">{structure.title}</h3>
          <div className="text-sm text-gray-500">
            {config.label} • {structure.estimatedWordCount} words • {structure.sections.length} sections
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gray-50 rounded-lg p-4">
            <h4 className="font-semibold text-gray-700 text-sm mb-2">Writing Approach</h4>
        ...
        {/* Sections */}
        <div>
          <h4 className="font-semibold text-gray-700 mb-3">Document Structure</h4>
      ...
      {isRegenerating && (
        <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-4">
          <h4 className="font-semibold text-gray-900 mb-4">Deep Regeneration in Progress...</h4>
          <ProgressStep
          ...
          <ProgressStep
            status={
              regenerationPhase === 'complete'
                ? 'completed'
                : regenerationPhase === 'regenerating' || regenerationPhase === 'researching'
                ? 'active'
                : 'pending'
            }
      ...
      {regenerationReport && !isRegenerating && (
        <RegenerationReport report={regenerationReport} onSourceToggle={handleSourceToggle} />
      )}
