...
type WriteModalAction =
  ...
  | {
      type: "SET_TOPIC_AND_INSTRUCTIONS";
      payload: {
        topic: string;
        instructions: string;
        wordCount: number | null;
...
const initialState: WriteModalState = {
  ...
  writingStyle: WritingStyle.ANALYTICAL, // DEFAULT
...
export default function WriteModal({ isOpen, onClose }: WriteModalProps) {
  ...
  const handleTopicSubmit = async (
    ...
    try {
      ...
      dispatch({
        type: "SET_ERROR",
        payload: error.message || "Failed to fetch sources",
      });
    } finally {
  ...
  const handleStructureConfirm = (
    ...
    if (updatedSources) {
      dispatch({
        ...
        payload: updatedSources.filter((s) => s.selected),
      });
    }
    dispatch({ type: "NEXT_STEP" });
  };
