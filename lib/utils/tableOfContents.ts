import { DocumentStructure, TableOfContents, TOCItem, WritingStyle, WRITING_STYLE_CONFIGS } from '@/lib/types/document';
  ...
  structure.sections.forEach((section, index) => {
    ...
    switch (writingStyle) {
      case WritingStyle.CHAPTER_BASED:
        title = `Chapter ${index + 1}: ${section.heading}`;
    ...
    items.push({
      level: 1,
      title,
      sectionNumber: writingStyle !== WritingStyle.NARRATIVE ? sectionNumber : undefined,
    });
    ...
    if (section.keyPoints && section.keyPoints.length > 0) {
      section.keyPoints.forEach((keyPoint, subIndex) => {
        ...
        const cleanedTitle = keyPoint.replace(/^\d+\.\d+\s+/, '').replace(/^\d+\.\s+/, '');
        ...
        items.push({
          level: 2,
          title: cleanedTitle,
          sectionNumber: writingStyle !== WritingStyle.NARRATIVE ? subSectionNumber : undefined,
        });
      });
    }
  });
 ...
 */
export function formatTOCForDocument(toc: TableOfContents, writingStyle: WritingStyle): string {
  let output = '\n## Table of Contents\n\n';
 ...
 */
export function generateTOCString(structure: DocumentStructure, writingStyle: WritingStyle): string {
